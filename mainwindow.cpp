...
void MainWindow::MainUIInit(){
    ...
    current_date_collect = current_date_time.toString(QString::fromLocal8Bit("yyyy-MM-dd hh mm")) + QString::fromLocal8Bit(" 头显 imu_camera 数据");
    //设置文件存放地址
    ...
    connect(this,SIGNAL(sig_imu_data_post(QList<string>)),m_hmd_about,SLOT(slot_setIMUData(QList<string>)));
    connect(this,SIGNAL(sig_camera_data_post(imrCameraData)),m_camera_data,SLOT(slot_setCameraData(imrCameraData)));
...
void MainWindow::showTimelimit_camera(){
    ...
    if(camera_temp._timeStamp != 0 && camera_temp._timeStamp != temp_time){
        emit sig_camera_data_post(camera_temp);
    }
    temp_time = camera_temp._timeStamp;
}
...
void MainWindow::showTimelimit_camera_show(){
    ...
    if(camera_temp._timeStamp != 0){
        ...
        namedWindow("left_camera",0);
        namedWindow("right_camera",0);

        cvResizeWindow("left_camera", 640, 400);
        cvResizeWindow("right_camera", 640, 400);

        imshow("left_camera", left);
